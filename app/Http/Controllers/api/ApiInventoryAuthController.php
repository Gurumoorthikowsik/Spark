...
class ApiInventoryAuthController extends Controller
{  
        ...
        if($_SERVER['REQUEST_METHOD'] === 'POST'){
           ...
           if($validator->fails()){
               ...
               $password = encrypt_decrypt('encrypt',trim($request['password']));  
                   ...
                   $check_user = User::select('email','password','userid','status','username','inventory_access','auth_key')->where('inventory_access',1)->where('email',$email)->where('password',$password);
